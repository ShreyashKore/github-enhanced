...
| The PAT following a redirect to a host that is not GitHub | Redirects are never handed to the JDK client, which would replay `Authorization` to whatever `Location` names. `GitHubClient.send` resolves each hop itself and requires the *same origin* — scheme, host and effective port — before re-issuing the request; anything else raises `GitHubError.CrossOriginRedirect` and no request is made. Same-origin hops (renamed repo, trailing slash) still follow, up to `MAX_REDIRECTS`. Covered by tests that assert the redirect target never sees the request. |
| The PAT sent over an unencrypted connection | `GitHubClient.checkedUri` parses the URL and throws `GitHubError.InsecureEndpoint` *before the token is read* for anything but `https`, with a loopback-literal exception for local test servers. The REST/GraphQL URL fields in settings enforce the same rule as a blocking validation error rather than the old warning. `127.example.com` is a registrable domain, not loopback, and is treated as such. |
| A PAT for one host being re-saved against another | The settings dialog loads the token for the persisted host; changing the host field leaves that secret in the (untouched) field. `PrCommentsConfigurable.apply` writes only a token the user actually edited, then reloads the field from the new host's own entry. Tokens for other hosts are left in `PasswordSafe` rather than deleted, which is what per-host keying is for. |
